...
res_base_dir=base_output_path+'heatmap_attspan_res/'

date = 23092020                               #1111111111111111111111  dd/mm/yyyy
timing = '18001900'					#2222222222222222222222  start time 6pm and end time 7 pm then(hhmmhhmm) --   18001900 

leng=3643

seqs = [date]  				#3333333333333333333333  date
lengths = [leng]  			#4444444444444444444444  no of frames under process
...
class GN():
    ...
    def initOut(self):
        print('     Loading Data...')
        self.train_set = DatasetFromFolder(sequence_dir, t_dir,tau_conf_score)
    ...
    def copyLines(self, seq, head, gt_seq, tail=-1, tag=0):
        ...
        if tt_tag:
            ...
            basic_dir = mot_dataset_dir + 'MOT%d/train/MOT%d-%02d/' % (year, year, seq)
    ...
    try:
        ...
        for t in types:
            ...
            for i in range(len(seqs)):
                ...
                print('The date:', seq_index,'Timings :',timing,'\nNo of frames under process :', seq_len)

                s_dir = f_dir + '%08d/' % seq_index
